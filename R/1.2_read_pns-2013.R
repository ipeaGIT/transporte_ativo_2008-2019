...
changeCols <- colnames(pns2013)[1:69]
...
pns2013[, dummyVehicle := data.table::fifelse(v2032 == "Nenhum", 0, 1)]
...
pns2013[,(changeCols1):= lapply(.SD, as.numeric), .SDcols = changeCols1]


# clean memory
#rm(list=setdiff(ls(), c("pns2013", "pns2013")))
gc(reset = T)
...
table(pns2013$v4745,exclude = FALSE)
is.factor(pns2013$v4745)
levels(pns2013$v4745)

# pns2013[, v4745 := factor(x = v4745
...
pns2013[is.na(P04101), P04101 := 0]
pns2013[P04102 == ".", P04102 := 0]
pns2013[P04301 == ".", P04301 := 0]
pns2013[P04302 == ".", P04302 := 0]
...
pns2013[, actv_commutetime := fifelse( is.na(P04101),0, P04101 * 60 + P04102)] # Active commute time
#active travel time to habitual activities, such as going to or taking someone to school 
# pns2013[, actv_traveltimehabacts := fifelse( is.na(P04301),0, P04301 * 60 + P04302)]
# pns2013[, total_actvtraveltime := actv_commutetime + actv_traveltimehabacts] ## total active travel time
# pns2013[, physicallyactive30 := fifelse(total_actvtraveltime >= 30,1,0)] # total active travel time >30 (1,0)
# pns2013[, physicallyactive15 := fifelse(total_actvtraveltime >= 15,1,0)] # total active travel time >15 (1,0)
# pns2013[, actv_commutetime45 := fifelse(actv_commutetime >= 45,1,0)] #commute time >30 (1,0)
# pns2013[, actv_commutetime30 := fifelse(actv_commutetime >= 30,1,0)] #commute time >30 (1,0)
# pns2013[, actv_commutetime15 := fifelse(actv_commutetime >= 15,1,0)] #commute time >30 (1,0)
...
table(pns2013$P040,exclude = FALSE)
table(pns2013$actv_commutetime30,exclude = FALSE)
...
pns2013[ C004 <17 , v4721 := sum( E01602, E01604, E01802,
...
pns2013[, decileBR:= as.numeric( cut(v4721
...
pns2013[, quintileBR:= as.numeric( cut(v4721
                                          , breaks = Hmisc::wtd.quantile(x = v4721
                                                                         , weights = V00292
...
pns2013[, quintileRegion:= as.numeric( cut(v4721
                                              , breaks = Hmisc::wtd.quantile(x = v4721
                                                                             , weights = V00292
                                                                             ,probs=seq(0, 1, by=0.2), na.rm=T),
...
pns2013[, quartileRegion:= as.numeric( cut(v4721
                                              , breaks = Hmisc::wtd.quantile(x = v4721
                                                                             , weights = V00292
...
pns2013[, quartileMetro:= as.numeric( cut(v4721
                                             , breaks = Hmisc::wtd.quantile(x = v4721
                                                                            , weights = V00292
                                                                            , probs = seq(0, 1, by=0.25), na.rm=T),
